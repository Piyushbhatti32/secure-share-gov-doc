...
class ShareService {
  constructor() {
    ...
    if (this.isBrowser) {
      ...
      // Use a global in-memory store on the server
      if (!globalThis.__shareStore) {
        globalThis.__shareStore = [];
      }
      this.shares = globalThis.__shareStore;
    }
  }
  ...
  saveShares() {
    try {
      if (this.isBrowser) {
        ...
        // Persist back to process memory for server usage
        globalThis.__shareStore = this.shares;
      }
    } catch (error) {
  ...
  async removeSharingAccess(shareIdOrDocumentId, email) {
    try {
      if (email) {
        // Remove by documentId + recipient email
        let updated = null;
        for (const share of this.shares) {
          if (
            share.documentId === shareIdOrDocumentId &&
            share.sharedWithEmails.includes(email) &&
            share.status === 'active'
          ) {
        ...
        // Remove by shareId
        const share = this.shares.find(s => s.id === shareIdOrDocumentId);
        if (!share) {
          throw new Error('Share not found');
        }
        share.status = 'removed';
...
export const removeSharingAccess = (shareId, email) => 
  shareService.removeSharingAccess(shareId, email);
