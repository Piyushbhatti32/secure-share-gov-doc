...
export const checkGoogleDriveConnection = async () => {
  try {
    ...
    // First check if user has Google OAuth tokens in Firebase claims
    const idTokenResult = await currentUser.getIdTokenResult();
    const hasGoogleOAuth = idTokenResult.claims?.google_oauth_connected;
    ...
    // Fallback: Check user profile for legacy connection status
    try {
